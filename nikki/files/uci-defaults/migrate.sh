...
# since v1.18.0
mixin_rule=$(uci -q get nikki.mixin.rule)
[ -z "$mixin_rule" ] && uci set nikki.mixin.rule=0

mixin_rule_provider=$(uci -q get nikki.mixin.rule_provider)
[ -z "$mixin_rule_provider" ] && uci set nikki.mixin.rule_provider=0

# since v1.19.0
mixin_ui_path=$(uci -q get nikki.mixin.ui_path)
[ -z "$mixin_ui_path" ] && uci set nikki.mixin.ui_path=ui

# Renaming rule match keys
uci show nikki | grep -E 'nikki.@rule\[[[:digit:]]+\].match=' | sed 's/nikki.@rule\[\([[:digit:]]\+\)\].match=.*/rename nikki.@rule[\1].match=matcher/' | uci batch

# since v1.19.1
proxy_fake_ip_ping_hijack=$(uci -q get nikki.proxy.fake_ip_ping_hijack)
[ -z "$proxy_fake_ip_ping_hijack" ] && uci set nikki.proxy.fake_ip_ping_hijack=0

# since v1.20.0
mixin_api_port=$(uci -q get nikki.mixin.api_port)
[ -n "$mixin_api_port" ] && {
    uci del nikki.mixin.api_port
    uci set nikki.mixin.api_listen=[::]:$mixin_api_port
}

mixin_dns_port=$(uci -q get nikki.mixin.dns_port)
[ -n "$mixin_dns_port" ] && {
...
# since v1.22.0
proxy_transparent_proxy=$(uci -q get nikki.proxy.transparent_proxy)
[ -n "$proxy_transparent_proxy" ] && {
    ...
    if [ "$proxy_access_control_mode" != "all" ]; then
        proxy_acl_ip=$(uci -q get nikki.proxy.acl_ip)
        [ -n "$proxy_acl_ip" ] && {
        ...
        proxy_acl_ip6=$(uci -q get nikki.proxy.acl_ip6)
        [ -n "$proxy_acl_ip6" ] && {
        ...
        proxy_acl_mac=$(uci -q get nikki.proxy.acl_mac)
        [ -n "$proxy_acl_mac" ] && {
